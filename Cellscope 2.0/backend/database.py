...
def get_project_components(project_id):
    ...
    NOTE: This only needs formulation_json and data_json for component names.
    Parquet hydration is NOT needed here — component names are stored in JSON, not parquet.
    """
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
        ...
        for formulation_json, data_json in results:
            ...
            # Only parse the JSON to extract formulation component names — no parquet hydration needed
            if data_json:
                try:
    ...
    with get_db_connection() as conn:
        ...
        if cells and len(cells) > 0:
            ...
            default_cell_values = {
                ...
                'cutoff_voltage_lower': first_cell.get('cutoff_voltage_lower'),
                'cutoff_voltage_upper': first_cell.get('cutoff_voltage_upper'),
